...
interface AttachmentFormProps {
  initialData: Event & { attachments: Attachment[] };
  eventId: string;
}
...
export const AttachmentForm = ({ initialData, eventId }: AttachmentFormProps) => {
  ...
  const onDelete = async (id: string) => {
    try {
      setDeletingId(id);
      await axios.delete(`/api/courses/${eventId}/attachments/${id}`);
    ...
    <div className="mt-6 border bg-slate-100 rounded-md p-4">
      ...
      {isEditing && (
        <div>
          ...
          />
          <div className="text-xs text-muted-foreground mt-4">Add anything your participant know about your event.</div>
        </div>
      )}
    </div>
  );
